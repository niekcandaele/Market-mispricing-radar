...
The mirrored Streamlit scaffold now includes a refresh-trust sidebar, resettable Radar filters, app-like Radar result cards with detail-focus and source actions, fuller Market Detail header context, visual score cards, readable supporting-signal tables, same-category peer comparison, explicit unavailable-state messaging, missing-field notes for thin detail payloads, and a methodology screen structured around the judge-facing trust notes.
