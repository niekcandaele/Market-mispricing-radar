...
Market Mispricing Radar helps an operator find prediction markets that deserve a second look. Instead of forcing someone to scan raw odds market by market, it ranks the markets that look most suspicious or fragile and explains why they surfaced.
...
The key design choice is honesty. This project does not pretend to know the perfect fair value of every market, and it does not claim guaranteed arbitrage. Instead, it prioritizes which markets look most worth inspecting right now and shows the evidence behind that prioritization.
...
4. score fragility signals like staleness, extremeness, weak support, and recent instability
5. generate market-level explanations
6. deploy the results into a judge-facing Streamlit app inside Zerve
...
- it balances technical depth with a judge-friendly product surface
...
- local Git / Forgejo project workflow for implementation tracking and artifact management

## Challenges we ran into

- Polymarket ingestion inside Zerve initially hit request-shape and header issues
- Zerve notebook and deploy automation was brittle through raw UI interaction, so the workflow had to be hardened with React-hook and internal API validation paths
...
- shipping a polished Radar / Detail / Methodology product flow for judges
- improving live category quality and fixing false-positive labeling in the deployed app
- packaging the project with a demo script, deck outline, and submission bundle docs instead of stopping at code completion
...
- deployed judge-facing app inside Zerve
...
- finalize the submission deck and office-layer presentation artifacts
